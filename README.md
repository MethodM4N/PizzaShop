...
- Выполнил оптимизацию сборки бандла (поделил страницы на отдельные чанки (действие когда потом подгружаем эти чанки называется lazy loading, React.lazy(app.ts), загрузка страниц только при запросе на соответствующий маршрут), создал наименования чанкам (/* webpackChunkName: "Cart" */))
...
- При помощи barrel export экспортировал компонент CartBlock в index.ts,
тоже хорошая практика как и reexport
```
